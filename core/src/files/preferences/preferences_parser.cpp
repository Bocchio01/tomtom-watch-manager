...
#include "tomtom/files/preferences/preferences_parser.hpp"
...
namespace tomtom::files::preferences
{
    ...
    namespace
    {
         ...
         */
        std::chrono::system_clock::time_point extractTimestamp(const std::string &xml, const std::string &tag)
        {
    ...
    models::Preferences PreferencesParser::parse(const std::vector<uint8_t> &xml_data)
    {
        // Convert to string
    ...
    models::Preferences PreferencesParser::parseString(const std::string &xml_string)
    {
        ...
        try
        {
            ...
            prefs.config_url = extractText(xml_string, "config_url");
