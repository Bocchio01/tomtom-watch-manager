...
#include "tomtom/interface/protocol/packet_header.hpp"
...
namespace tomtom::interface::codec
{
    ...
    public:
         ...
         */
        static void validateHeader(const protocol::PacketHeader &header)
        {
            if (header.direction != protocol::PacketDirection::RX)
            {
        ...
        static void expectType(const protocol::PacketHeader &header)
        {
            using Traits = protocol::PayloadTraits<typename ExpectedPacket::PayloadType>;
            ...
            constexpr protocol::MessageType expected = Traits::type;
