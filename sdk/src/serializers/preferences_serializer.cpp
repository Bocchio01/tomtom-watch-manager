...
namespace tomtom::sdk::serializers
{
    ...
    std::vector<uint8_t> PreferencesSerializer::serialize(const Preferences &prefs)
    {
        std::string xml = buildXML(prefs);
        return std::vector<uint8_t>(xml.begin(), xml.end());
    }

    std::string PreferencesSerializer::buildXML(const Preferences &prefs)
    {
        ...
        {
            xml << "  <ephemeris_url>" << prefs.ephemeris_url << "</ephemeris_url>\n";
        }
